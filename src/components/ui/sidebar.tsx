...
const NavItem: React.FC<{
  ...
  isExpanded: boolean;
  pathname: string;
}> = ({ item, isExpanded, pathname }) => {
  const isActive = pathname.startsWith(item.to);
  return (
    <div className="relative w-full">
      <Link
        href={item.to}
        className={`flex items-center gap-4 w-full h-12 px-4 rounded-lg transition-colors duration-200 ${
          isActive ? "bg-primary/10" : "hover:bg-gray-100"
        }`}
      >
        <span
          className={`text-2xl ${isActive ? "text-primary" : "text-sky-900"}`}
        >
          {item.icon}
        </span>
        <AnimatePresence>
      ...
      >
        ...
        <div className="flex flex-col gap-2">
          <div className="group relative">
            <div
              onClick={() => clearAuth()}
              className="flex items-center gap-4 w-full h-12 px-4 rounded-lg cursor-pointer hover:bg-gray-100"
            >
            ...
            {!isExpanded && (
              <div className="absolute left-full ml-4 px-2 py-1 bg-gray-800 text-white text-xs rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
                Logout
              </div>
            )}
          </div>
          ...
          <button
            ...
            className="flex items-center gap-4 w-full h-12 px-4 rounded-lg cursor-pointer hover:bg-gray-100"
          >
            <span className="text-2xl text-sky-900">
